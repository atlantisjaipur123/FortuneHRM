...
async function getPfEsiRulesClient(
  rateType: "PF" | "ESI" | "GRATUITY"
) {
...
const buildDefaultForm = (columns: ColumnConfig[]) => {
  const form: Record<string, string | number> = {};
  columns.forEach((col) => {
    form[col.key] = col.defaultValue;
  });
  return form;
};
...
export default function PFESIRatesSetupPage() {
  ...
  const [esiForm, setESIForm] = useState<Partial<ESIRate>>(buildDefaultForm(initialESIColumns));
  ...
  const [pfForm, setPFForm] = useState<Partial<PFRate>>(buildDefaultForm(initialPFColumns));
  ...
  const [gratuityForm, setGratuityForm] = useState<GratuityForm>({
    gratuityPercent: initialGratuityColumns[0].defaultValue as number,
  });
  ...
  const handleGratuitySubmit = async () => {
    ...
    const payload = {
      rateType: "GRATUITY",
      gratuityPercent: Number(gratuityForm.gratuityPercent),
    ...
    setGratuityForm({ gratuityPercent: initialGratuityColumns[0].defaultValue as number, });
  ...
  const handleESISubmit = async () => {
    try {
      const payload = {
        rateType: "ESI",
        empShare: Number(esiForm.empShare),
        employerShare: Number(esiForm.employerShare),
        esiWageCeiling: Number(esiForm.wageCeiling),
      ...
      setESIForm(buildDefaultForm(esiColumns));
  ...
  const handleESIChange = (field: string, value: string | Date) => {
    if (
      typeof value === "string" &&
      ["empShare", "employerShare"].includes(field)
    ) {
      const num = Number(value);
      if (!isNaN(num) && num > 100) return;
    }
  ...
  const handlePFSubmit = async () => {
    try {
      const payload = {
        rateType: "PF",
        empShareAc1: Number(pfForm.empShareAc1),
        erShareAc2: Number(pfForm.erShareAc2),
        adminChargesAc10: Number(pfForm.adminChargesAc10),
        epsAc21: Number(pfForm.epsAc21),
        edliChargesAc21: Number(pfForm.edliChargesAc21),
        adminChargesAc22: Number(pfForm.adminChargesAc22),
        calcType: pfForm.calcType,
        pfWageCeiling: Number(pfForm.wageCeiling),
        epsWageCeiling: Number(pfForm.epsWageCeiling),
        minEpsContribution: Number(pfForm.minEpsContribution),
      ...
      setPFForm(buildDefaultForm(pfColumns));
              ...
              {gratuityColumns.map((col) => (
                <div key={col.key}>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleGratuityChange("gratuityPercent", e.target.value)
                    }
                    onFocus={(e) => e.target.select()}
                    onKeyDown={(e) => {
                      if (["e", "E", "+", "-"].includes(e.key)) {
              ...
              <TableBody>
                {gratuityRates.map((rate, index) => (
                  <TableRow
                    ...
                    className={`cursor-pointer ${selectedGratuityRate?.id === rate.id ? "bg-blue-50" : ""
                      }`}
                  >
